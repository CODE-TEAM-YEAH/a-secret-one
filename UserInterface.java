...
public class UserInterface {
	...
	private void Page1() { // Function for page 1.
		...
		int choice = in.nextInt(); // Reads in the user's option for page choices.
	...
	private void Page5() { // Function for page 5.
		...
		if (choice2 == 1) { // If they chose to view items by category.
			...
			File g = new File("ItemsBought_" + cart.getUsername() + ".txt");
			if (!g.exists()) {						
				System.out.println("\nNo items purchased.");
				changeCurrentPage(5);
			}
				...
				System.out.format("%10s%30s%10s\n", "Confirmation Id", "Product Name", "Total");
				String Line = null;
				try {
	...
	private void Page10() {
		...
		String ShopChoice = in.next();
		
		double randNumber = Math.random();
		double d = randNumber * 100;
		int randomInt = (int)d;
		String ID = "UD1" + randomInt;
		
		if (ShopChoice.toLowerCase().equals("yes")){
			System.out.println("\nConfirmation ID: " + ID);
			System.out.println("Items shipped to Mr." + userName + ".\nReturning to menu . . .");
			cart.addToPurchased(total, ID);
			cart.clean();
			changeCurrentPage(5);
		}
